...
local emp = LibStub("AceAddon-3.0"):NewAddon("EpicMusicPlayer", "AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0",
    "AceComm-3.0")
...
emp.controlslist = {
    TogglePlay = L["Play/Stop"],
    OnNextClick = L["Play Next Song"],
    OpenMenu = L["Drop Down Menu"],
    ShowConfig = L["Config"],
    ToggleMute = L["Mute / unmute music sound."],
    TogglePlayListGui = L["Playlist"],
    PlayLast = L["Play Last Song"],
    TogglePlayerGui = L["Toggle show GUI"],
    ToggleRandom = L["Toggle shuffle"],
    RemoveCurrendSong = L["Remove Song"],
    SpamDefault = L["Send song name to default channel"],
}
...
function emp:OnEnable(first)
    ...
    self:RegisterEvent("ZONE_CHANGED_INDOORS", emp.OnZoneChangedNewArea)
    self:RegisterEvent("ZONE_CHANGED", emp.OnZoneChangedNewArea)
...
-- event functions
------------------------------------------------------------------------------
local function printZoneEvent(listName, subZoneMatched, zone, subZone)
    if db.printZoneEvents then
        if subZoneMatched then
            print(string.format(L["EpicMusicPlayer: Playing song from list [%s] for sub-zone [%s]"], listName, subZone))
        else
            print(string.format(L["EpicMusicPlayer: Playing song from list [%s] for zone [%s]"], listName, zone))
        end
    end
end
...
function emp:OnZoneChangedNewArea(event)
    if db.enableEvents then
        ...
        emp:Debug("subZone:", subZone)
        local match
        local subZoneMatched
        for name, key in pairs(db.eventZones) do
            if subZone == name then
                match = key
                subZoneMatched = true
            end
        end
        -- no subzone found lets try zone match
        if not match then
            emp:Debug("subZone no match lets try zone...")
            for name, key in pairs(db.eventZones) do
                if zone == name then
                    match = key
                    emp:Debug("Zone match", match)
                end
            end
        end
        if match then
            emp:Debug("match:", zone, subZone, match)
            local _, listIndex = emp:_GetListByName(match)
            if db.list ~= listIndex then
                _, db.list, db.song = emp:GetRandomSong(listIndex)
                emp:Play()
                printZoneEvent(match, subZoneMatched, zone, subZone)
            end
            return 1
        end
    end
